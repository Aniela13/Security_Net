...
CREATE PROCEDURE usp_AddOperator(
  ...
  BEGIN
		INSERT INTO USER_NET(NAME, LASTNAME, DNI, USERNAME, PASSWORD, AUTHORIZED, HELPNEEDED, USER_TYPE, DOCUMENT_TYPE, BIRTHDAY, ADDRESS, GENDER, PHONE_NUMBER, PHOTO, SALARY, SCHEDULE,HIRE_DATE,EMAIL)
		SELECT @NAME, @LASTNAME, @DNI, @USERNAME,@PASSWORD, @AUTHORIZED, @HELPNEEDED, @USER_TYPE, @DOCUMENT_TYPE, @BIRTHDAY, @ADDRESS, @GENDER, @PHONE_NUMBER, @PHOTO, @SALARY, @SCHEDULE,@HIRE_DATE,@EMAIL
...
END
GO
CREATE PROCEDURE usp_QueryAllFAQ AS
	BEGIN
		SELECT *
		FROM QUESTION
		WHERE FAQ='1'
...
END
GO
CREATE PROCEDURE usp_QueryAllNewQuestions AS
	BEGIN
		SELECT *
		FROM QUESTION
...
END
GO
CREATE PROCEDURE usp_QueryAnswerByQuestion(
    @QUESTION         VARCHAR
) AS 
	BEGIN
