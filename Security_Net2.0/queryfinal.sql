...
IF OBJECT_ID('dbo.USER_NET','U') IS NOT NULL	
	drop table USER_NET;
IF OBJECT_ID('dbo.WARNING_TYPE','U') IS NOT NULL	
    drop table WARNING_TYPE;	
--SENTENCIA SQL DDL para crear la tabla Tipos de Documento
CREATE TABLE DOCUMENT_TYPE (
   ID_TYPE CHAR(1) NOT NULL PRIMARY KEY,
...
CREATE TABLE USER_NET (
	...
	EMAIL			VARCHAR(50),
	SATISFACTION	VARCHAR(1) NULL, -- 1: Good, 0: Bad
	STATUS			CHAR(2) NULL -- A: habilitado / U: Deshabilitado-eliminado
);
GO
ALTER TABLE USER_NET
ADD CONSTRAINT FK_DOCUMENT_TYPE_ID
FOREIGN KEY (DOCUMENT_TYPE)
REFERENCES DOCUMENT_TYPE(ID_TYPE);
GO
...
END
GO*/
--Sentencia SQL DDL (data definition language) para crear la tabla WARNING ASOCIADA A WARNING_TYPE
...
CREATE TABLE WARNING(
	...
	START_DATE		DATETIME,
	END_DATE		DATETIME,
	WARNING_TYPE    INT NOT NULL, -- Categorías insertadas en la tabla WARNING_TYPE
	DESCRIPTION		VARCHAR(500),
	ZONE 			VARCHAR(50) NULL,
	ACTIVE		    VARCHAR(3) -- Alarm active or not
...
GO
--  Sentencia SQL DML (data modifying language) para insertar tipos de documentos
INSERT INTO DOCUMENT_TYPE (ID_TYPE, NAME_TYPE) VALUES
('0', 'DNI'),
('1', 'PAS'), 
('2', 'CE');
GO
--  Sentencia SQL DML (data modifying language) para insertar registros a la tabla WARNING_TYPE
INSERT INTO WARNING_TYPE (ID, NAME, DESCRIPTION) VALUES
(1, 'Falsa alarma', 'La alarma fue descartada por el operador'),
(2, 'Alarma de pánico' , 'Botón de emergencia presionado por un usuario'),
(3, 'Comportamiento sospechoso' , 'Personas merodeando sin intención de compra o intento de acceso a zonas no autorizadas'),
...
-- Sentencia SQL para consultar todos los tipos de documentos
IF EXISTS(  SELECT * 
			FROM sysobjects
			WHERE id = object_id(N'[dbo].[usp_QueryAllDocumentTypes]')
			  AND OBJECTPROPERTY(id, N'IsProcedure') = 1 )
BEGIN
	DROP PROCEDURE [dbo].usp_QueryAllDocumentTypes
END
GO
CREATE PROCEDURE dbo.usp_QueryAllDocumentTypes AS
	BEGIN
		SELECT *
		FROM DOCUMENT_TYPE
	END
...
CREATE PROCEDURE usp_DeleteOperator(
	...
	BEGIN
		UPDATE USER_NET
		SET STATUS = 'U' 
		WHERE ID=@ID AND USER_TYPE='1' AND AUTHORIZED='SI'
	END
GO
...
CREATE PROCEDURE dbo.usp_QueryAllFAQ AS
	BEGIN
		SELECT *
		FROM QUESTION
		WHERE FAQ='1'AND STATUS = 'Vigente'
	END
GO
...
CREATE PROCEDURE dbo.usp_QueryAllNewQuestions AS
	BEGIN
		SELECT *
		FROM QUESTION
		WHERE FAQ='0' AND ANSWER = 'Por definir' AND STATUS = 'Vigente'-- las preguntas sin respuesta son nuevas
	END
GO
...
IF EXISTS(  SELECT * 
			...
			WHERE id = object_id(N'[dbo].[usp_QueryActivatedWarnings]')
			  AND OBJECTPROPERTY(id, N'IsProcedure') = 1 )
BEGIN
	DROP PROCEDURE [dbo].usp_QueryActivatedWarnings
END
GO
