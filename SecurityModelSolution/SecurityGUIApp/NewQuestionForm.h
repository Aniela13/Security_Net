...
#pragma endregion
	...
	private: System::Void btnAsk_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		Controller::AddNewQuestion(newq); 
		btnAsk->Enabled = false;
		String^ answerbyop = Controller::QueryAnswerByQuestion(newq);
		while (answerbyop == nullptr) {
			if (answerbyop != nullptr) {
				break;
			}
		}
		txtAnswerbyOp->Text = answerbyop;
	}
};
}